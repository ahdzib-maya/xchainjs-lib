...
describe('Client Test', () => {
  ...
  const testnet_address_path0 = 'tmaya19kacmmyuf2ysyvq3t9nrl9495l5cvktj50t4p9'
  const testnet_address_path1 = 'tmaya1hrf34g3lxwvpk7gjte0xvahf3txnq8ecvaxfud'
  ...
  it('has balances', async () => {
    ...
    // mainnet - 0: thor19kacmmyuf2ysyvq3t9nrl9495l5cvktjsc69cq
  ...
  it('rune + synth balances', async () => {
    ...
    const balances = await thorClient.getBalance('tmaya13gym97tmw3axj3hpewdggy2cr288d3qff5euqc')
  ...
  it('filter BUSD synth balances', async () => {
    ...
    const balances = await thorClient.getBalance('tmaya13gym97tmw3axj3hpewdggy2cr288d3qff5euqc', [BUSD_ASSET_SYNTH])
    expect(balances.length).toEqual(1)
    // BUSD synth
  ...
  it('has tx history', async () => {
    const historyData = require('../__mocks__/responses/tx_search/sender-tmaya137kees65jmhjm3gxyune0km5ea0zkpnj4lw29f.json')
    ...
    const address = 'tmaya137kees65jmhjm3gxyune0km5ea0zkpnj4lw29f'
    ...
    const txs = await thorClient.getTransactions({
      address: 'tmaya137kees65jmhjm3gxyune0km5ea0zkpnj4lw29f',
    })
  ...
  it('transfer', async () => {
    const to_address = 'tmaya19kacmmyuf2ysyvq3t9nrl9495l5cvktj50t4p9'
  ...
  it('should return valid explorer url', () => {
    expect(thorClient.getExplorerUrl()).toEqual('https://stagenet.explorer.mayachain.info?network=testnet')

    thorClient.setNetwork(Network.Mainnet)
    expect(thorClient.getExplorerUrl()).toEqual('https://stagenet.explorer.mayachain.info')
  })

  it('should return valid explorer address url', () => {
    expect(thorClient.getExplorerAddressUrl('tmayaabc')).toEqual(
      'https://stagenet.explorer.mayachain.info/address/tmayaabc?network=testnet',
    )

    thorClient.setNetwork(Network.Mainnet)
    expect(thorClient.getExplorerAddressUrl('thorabc')).toEqual(
      'https://stagenet.explorer.mayachain.info/address/thorabc',
    )
  })

  it('should return valid explorer tx url', () => {
    expect(thorClient.getExplorerTxUrl('txhash')).toEqual(
      'https://stagenet.explorer.mayachain.info/tx/txhash?network=testnet',
    )

    thorClient.setNetwork(Network.Mainnet)
    expect(thorClient.getExplorerTxUrl('txhash')).toEqual('https://stagenet.explorer.mayachain.info/tx/txhash')
  })
